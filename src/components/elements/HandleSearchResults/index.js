...
const getPostCodesAndAddToList = (params = {}, callback = () => { }) => {
    ...
    fetch(url, config)
        ...
        .catch((error) => {
            let message = "EDINBURG TRANSFERS  HandleSearchResults Component - getPostCodesAndAddToList function      fetching catch blog  "
...
const requestForGooglePLace = (params = {}, callback = () => { }) => {
    ...
    fetch(url, config)
        ...
        .then((data) => { callback(data) })
        .catch((error) => {
            let message = "EDINBURG TRANSFERS  HandleSearchResults Component - requestForGooglePLace function      fetching catch blog  "
            window.handelErrorLogs(error, message, { config, url })
        });
}
